...
import { journeySteps, passions } from "../../utils/data";
...
const About = () => {
    ...
    return (
        <section ref = {sectionRef} className = {`py-24 px-6 ${isDarkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"} relative overflow-hidden`} id = "about">
            ...
            <div className = "max-w-6xl mx-auto relative z-10">
                <motion.div initial = "hidden" animate = {isInView ? "visible" : "hidden"} variants = {containerVariants} className = "text-center mb-20">
                    ...
                    <motion.h2 variants = {itemVariants} className = "text-3xl md:text-5xl font-light mb-6">
                        Sobre <span className = "text-blue-500 font-medium">mí</span>
                    </motion.h2>
                </motion.div>
                <div className = "grid lg:grid-cols-2 gap-16 items-start">
                    <motion.div initial = "hidden" animate = {isInView ? "visible" : "hidden"} variants = {containerVariants} className = "space-y-8">
                        <motion.div variants = {itemVariants} className = {`p-8 rounded-2xl border backdrop-blur-sm ${isDarkMode ? "bg-gray-800/50 border-gray-700" : "bg-gray-50/80 border-gray-200"}`}>
                            <h3 className = "text-2xl font-medium mb-6">
                                Mi misión
                            </h3>
                            <p className = {`text-lg leading-relaxed mb-6 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                                Creo que la tecnología es el puente que une ideas, personas y sueños, una herramienta capaz de transformar vidas y crear conexiones que trascienden fronteras, por eso me apasiona construir soluciones digitales que inspiren, comuniquen y acerquen a las personas al mundo que imaginan, convirtiendo cada línea de código en una oportunidad para conectar humanidad e innovación.
                            </p>
                            <p className = {`text-base leading-relaxed ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
                                Me motiva explorar nuevas tecnologías, contribuir al crecimiento de proyectos innovadores y evolucionar constantemente en el mundo de la web, creando experiencias digitales únicas, además disfruto guiar y mentorizar a aspirantes a desarrolladores web para compartir conocimientos y fomentar su crecimiento profesional.
                            </p>
                        </motion.div>
                        <motion.div variants = {itemVariants} className = "space-y-4">
                            <h3 className = "text-xl font-medium mb-6">
                                Lo que me apasiona crear
                            </h3>
                        ...
                        <motion.div variants = {itemVariants} className = "text-center py-8">
                            <div className = {`text-sm ${isDarkMode ? "text-gray-500" : "text-gray-600"} mb-4`}>
                                Creado con pasión y propósito por
                            </div>
                            <div className = "flex justify-center">
                                <img src = {signature} alt = "Carlos" className = "w-28" />
                            </div>
                            <div className = "text-lg font-medium text-blue-500 mt-4">
                                Carlos E. Silva Bustamante
                            </div>
                        </motion.div>
                    </motion.div>
                    <motion.div ref = {timelineRef} initial = "hidden" animate = {timelineInView ? "visible" : "hidden"} variants = {timelineVariants} className = "relative">
                        ...
                        <div className = {`absolute top-16 bottom-0 left-8 w-px ${isDarkMode ? "bg-gray-700" : "bg-gray-300"}`} />
                        <div className = "space-y-8">
                            {
                                [...journeySteps]
                                .sort((a, b) => b.year - a.year)
                                .map((step, i) => (
                                    <motion.div key = {i} variants = {stepVariants} whileHover = {{ x: 4 }} className = "flex items-start relative space-x-6 group">
                                        <div className = {`flex items-center justify-center flex-shrink-0 relative w-16 h-16 rounded-full ${step.color} group-hover:scale-110 transition-transform duration-300`}>
                                            <step.icon size = {24} className = "text-white" />
                                        </div>
                                        <div className = {`flex-grow p-6 rounded-xl border transition-all duration-300 ${isDarkMode ? "bg-gray-800/50 border-gray-700 group-hover:border-gray-600 group-hover:bg-gray-800" : "bg-white/80 border-gray-200 group-hover:border-gray-300 group-hover:bg-white" } backdrop-blur-sm`}>
